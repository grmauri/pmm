...
//#define DBG           // modo "debug" (mostra a converg�ncia dos m�todos)
...
int main(int argc, char *argv[])
{
        ...
        double mut = 7;
        double eli = 15;
        // VNS e ILS sem par�metros espec�ficos
        //---
        // usar os valores passados por par�metro; caso contr�rio, usar� os valores definidos acima
        if (argc > 1)
        {
            seed = stoi(argv[1]);
            metodo = stoi(argv[2]);
...
void busca_tabu(const int tam_lista, const double tempo_max, Solucao &s, double &tempo_melhor, double &tempo_total)
{
    ...
    while(tempo_total < tempo_max)
    {
        ...
        melViz = -INT_MAX;
        flag = -1; // todos os vizinhos na lista e n�o h� melhora
        for(int j = 0; j < numObj; j++)
        {
            ...
            for(int i = -1; i < numMoc; i++)
            {
                if(i != s_vizinha.vetSol[j])
                {
                    ...
                    if(pos != -1) // est� na lista tabu
                    {
                        if(s_vizinha.funObj > s.funObj) // vizinha melhor que a melhor
                        {
                            flag = 0; // est� na lista tabu, mas � melhor do que a melhor
                            melViz = s_vizinha.funObj;
                            melMoc = i;
                            melObj = j;
                            melPos = pos; // posi��o na lista tabu
                        }
                    }
                    else // n�o est� na lista
                    {
                        ...
                        {
                            flag = 1; // melhor vizinha que n�o est� na lista tabu
                            melViz = s_vizinha.funObj;
                            melMoc = i;
                            melObj = j;
                            melPos = -1; // n�o est� na lista tabu
                        }
                    }
                }
            }
        ...
        if(flag == -1) // todos os vizinhos na lista e nenhuma melhora encontrada, ent�o aplica-se o primeiro movimento da lista (FIFO)
        {
        ...
        {
            if(flag == 0) // vizinha melhor que a melhor solu��o, ent�o remove-se o movimento que gerou a vizinha
                remover_lista(lista_tabu, qtd_lista, melPos);
            else // movimento que gerou a vizinha n�o est� na lista, ent�o deve-se inserir
                inserir_lista(lista_tabu, qtd_lista, tam_lista, melObj, melMoc);
            s_vizinha.vetSol[melObj] = melMoc;
            s_vizinha.funObj = melViz;
        }
        //--------------------------------------
        // Atualizar a melhor solu��o
        if(s_vizinha.funObj > s.funObj)
        {
            memcpy(&s, &s_vizinha, sizeof(s_vizinha));
            hF = clock();
